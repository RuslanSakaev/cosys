...
import ru.sakaev.backend.model.Product; // Импорт класса модели Product
import ru.sakaev.backend.service.ProductService; // Импорт класса ProductService
...
public class ProductController { // Объявление класса ProductController
    ...
    public ResponseEntity<Product> updateProduct(@PathVariable Long id, @RequestBody Product product) { // Метод обновления продукта
        ...
        int newQuantity = 0;
        Product updatedProduct = productService.updateProduct(id, product, newQuantity); // Обновление продукта через сервис
